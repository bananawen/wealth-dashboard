            ...
            {['buy', 'sell'].map(t => (
              <button
                ...
                className={`flex-1 py-2 rounded-lg font-medium transition-colors ${
                  form.type === t
                    ? t === 'buy' ? 'bg-red-500 text-white' : 'bg-emerald-500 text-white'
                    : 'bg-[var(--bg-secondary)]'
                }`}
              >
                {t === 'buy' ? '買入' : '賣出'}
          ...
          <div className="flex flex-wrap gap-1 mt-2">
            {quickDates.map(qd => (
              <button
                ...
                className="px-2 py-1 text-xs rounded border border-[var(--border-color)] hover:border-blue-500 transition-colors"
              >
                {qd.label}
              </button>
            ))}
